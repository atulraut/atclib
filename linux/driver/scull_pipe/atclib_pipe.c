 ...
 * To test
 * 1. shell_1 -> cat /dev/atclibp0
 * 2. shell_2 -> echo "Raje" >/dev/atclibp0
 */
...
static struct class *atclibp_class;
			...
			loff_t *f_pos)
{
	...
	while (dev->rp == dev->wp) { /* nothing to read */
		...
		if (filp->f_flags & O_NONBLOCK)
			return -EAGAIN;
		pr_err("atclib: %s reading: going to sleep\n", current->comm);
		/*ERESTARTSYS: signal: tell fs layer to handle it*/
	...
	pr_err("atclib: %s did read %li bytes\n", current->comm, (long)count);
	return count;
}
...
static int scull_getwritespace(struct scull_pipe *dev, struct file *filp)
{
	while (spacefree(dev) == 0) { /* full */
		...
		if (filp->f_flags & O_NONBLOCK)
			return -EAGAIN;
		pr_err("atclib: %s writing: going to sleep\n", current->comm);
			...
			loff_t *f_pos)
{
	...
	/* scull_getwritespace called mutex_unlock(&dev->mutex) */
	...
	pr_err("atclib: Going to accept %li bytes to %p from %p\n",
	...
	pr_err("atclib: %s did write %li bytes\n", current->comm, (long)count);
	return count;
}
 ...
 * Set up a cdev entry.
 */
static int scull_p_setup_cdev(struct scull_pipe *dev, int index,
			       struct class *class)
{
	int ret = 0;
	int err;
	...
	devno = MKDEV(scull_p_devno, index);
	cdev_init(&dev->cdev, &scull_pipe_fops);
	dev->cdev.owner = THIS_MODULE;
	err = cdev_add(&dev->cdev, devno, 1);
	/* Fail gracefully if need be */
	if (err)
		pr_err("Error %d adding scullpipe%d", err, index);
	device = device_create(class, NULL,
				devno,
				NULL, DEVICE_NAME"%d", index);
	if (IS_ERR(device)) {
...
int scull_p_init(dev_t firstdev)
{
	...
	if (result < 0) {
		pr_err("Unable to get scullp region, error %d\n", result);
		return 0;
	}
	...
	atclibp_class = class_create(THIS_MODULE, CLASS_NAME);
	if (IS_ERR(atclibp_class)) {
