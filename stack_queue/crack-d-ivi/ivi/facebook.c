...
size_t cbq_write(struct CircularByteQueue *q, const unsigned char *data, size_t len) {
  ...
  if (first_part > bytes_to_write) first_part = bytes_to_write;
...
size_t cbq_read(struct CircularByteQueue *q, unsigned char *dest, size_t len) {
  ...
  if (bytes_to_read > q->size) bytes_to_read = q->size;

  size_t first_part = q->capacity - q->head;
  if (first_part > bytes_to_read) first_part = bytes_to_read;
...
int main() {
  ...
  unsigned char data[] = "hello world"; // 11 bytes
