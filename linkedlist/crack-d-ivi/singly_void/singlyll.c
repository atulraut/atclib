...
/* Implemented using while FOR */
void m_Freelist(void *ptr) {
  struct list *ll = (struct list *)ptr;
  struct list_node *listptr = (struct list_node*)ll->head;
...
bool hasCycle(void *ptr) {
  struct list *ll = (struct list *)ptr;
  if (ll->head == NULL) {
    return false;
  }

  struct list_node *slow = ll->head;
  struct list_node *fast = ll->head->next;
  while (slow != fast) {
